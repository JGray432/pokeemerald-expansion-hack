 ...
 * Task_NewGameBirchSpeech_StartBirchElectrikePlatformFade
 * Task_NewGameBirchSpeech_StartBirchElectrikePlatformFade
 ...
 * Task_NewGameBirchSpeech_ReshowBirchElectrike
 ...
 *  - Advances to Task_NewGameBirchSpeechSub_WaitForElectrike
 * Task_NewGameBirchSpeechSub_WaitForElectrike
...
static void Task_NewGameBirchSpeechSub_WaitForElectrike(u8);
static void Task_NewGameBirchSpeech_StartBirchElectrikePlatformFade(u8);
...
static void Task_NewGameBirchSpeech_ReshowBirchElectrike(u8);
...
#define tElectrikeSpriteId data[9]
...
{
    u8 spriteId = gTasks[sBirchSpeechMainTaskId].tElectrikeSpriteId;
    ...
    CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 112, 58, 0, 0, 32, PALETTES_BG, SPECIES_ELECTRIKE);
    gTasks[taskId].func = Task_NewGameBirchSpeechSub_WaitForElectrike;
    gTasks[sBirchSpeechMainTaskId].tTimer = 0;
}

static void Task_NewGameBirchSpeechSub_WaitForElectrike(u8 taskId)
{
    s16 *data = gTasks[taskId].data;
    struct Sprite *sprite = &gSprites[gTasks[sBirchSpeechMainTaskId].tElectrikeSpriteId];
...
static void Task_NewGameBirchSpeech_AndYouAre(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        ...
        AddTextPrinterForMessage(TRUE);
        gTasks[taskId].func = Task_NewGameBirchSpeech_StartBirchElectrikePlatformFade;
    }
}

static void Task_NewGameBirchSpeech_StartBirchElectrikePlatformFade(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        gSprites[gTasks[taskId].tElectrikeSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static void Task_NewGameBirchSpeech_StartPlayerFadeIn(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[gTasks[taskId].tElectrikeSpriteId].invisible = TRUE;
        if (gTasks[taskId].tTimer)
        {
            gTasks[taskId].tTimer--;
        }
...
static void Task_NewGameBirchSpeech_StartNamingScreen(u8 taskId)
{
    ...
    {
        FreeAllWindowBuffers();
        FreeAndDestroyMonPicSprite(gTasks[taskId].tElectrikeSpriteId);
...
static void Task_NewGameBirchSpeech_SlidePlatformAway2(u8 taskId)
{
    ...
    {
        gTasks[taskId].func = Task_NewGameBirchSpeech_ReshowBirchElectrike;
    }
}

static void Task_NewGameBirchSpeech_ReshowBirchElectrike(u8 taskId)
{
    u8 spriteId;
    ...
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        spriteId = gTasks[taskId].tElectrikeSpriteId;
        gSprites[spriteId].x = 100;
        gSprites[spriteId].y = 75;
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static void Task_NewGameBirchSpeech_WaitForSpriteFadeInAndTextPrinter(u8 taskId)
{
    ...
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        gSprites[gTasks[taskId].tElectrikeSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        if (!RunTextPrintersAndIsPrinter0Active())
        {
            gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
            gSprites[gTasks[taskId].tElectrikeSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static u8 NewGameBirchSpeech_CreateElectrikeSprite(u8 x, u8 y)
{
    return CreateMonPicSprite_Affine(SPECIES_ELECTRIKE, FALSE, 0, MON_PIC_AFFINE_FRONT, x, y, 14, TAG_NONE);
}

static void AddBirchSpeechObjects(u8 taskId)
{
    u8 birchSpriteId;
    u8 ElectrikeSpriteId;
    u8 brendanSpriteId;
    u8 maySpriteId;
    ...
    ElectrikeSpriteId = NewGameBirchSpeech_CreateElectrikeSprite(100, 0x4B);
    gSprites[ElectrikeSpriteId].callback = SpriteCB_Null;
    gSprites[ElectrikeSpriteId].oam.priority = 0;
    gSprites[ElectrikeSpriteId].invisible = TRUE;
    gTasks[taskId].tElectrikeSpriteId = ElectrikeSpriteId;
