...
enum {
    ...
    FIELD_MOVE_ROCK_SMASH,        // FLAG_BADGE05_GET
    FIELD_MOVE_DIVE,         // FLAG_BADGE06_GET
    FIELD_MOVE_WATERFALL,        // FLAG_BADGE07_GET
    FIELD_MOVE_STRENGTH,   // FLAG_BADGE08_GET
    FIELD_MOVE_TELEPORT,
    FIELD_MOVE_DIG,
...
void ItemUseCB_RareCandy(u8 taskId, TaskFunc task)
{
    ...
    if (cannotUseEffect)
    {
        ...
        if (holdEffectParam == 0 || holdEffectParam == 1 || holdEffectParam == 2)
        {
    ...
    else
    {
        ...
        if((holdEffectParam != 1 && holdEffectParam != 2))
            RemoveBagItem(gSpecialVar_ItemId, 1);
        GetMonNickname(mon, gStringVar1);
        if (sFinalLevel > sInitialLevel)
        {
            PlayFanfareByFanfareNum(FANFARE_LEVEL_UP);
            if (holdEffectParam == 0 || holdEffectParam == 1 || holdEffectParam == 2) // Rare Candy & Inf Candy
            {
...
void ChooseMonForStatus(void)
{
    InitPartyMenu(PARTY_MENU_TYPE_FIELD, PARTY_LAYOUT_SINGLE,  PARTY_ACTION_CHOOSE_AND_CLOSE, FALSE, PARTY_MSG_CHOOSE_MON, Task_HandleChooseMonInput, CB2_ReturnFromChooseMonForStatus);
}

void ChooseMonForIVMax(void)
{
